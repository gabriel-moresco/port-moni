...
namespace PortMoni.VIEWMODEL
{
    public class NewOperationViewModel : ObjectNotification
    {
        ...
        void SaveNewInvestment(object parameter)
        {
            ...
            Operation operation = new Operation(ShareCode, operationType, Quantity, OperationPrice, DateTime.Parse(OperationDate), OperationTaxes);
        ...
        {
            return (BuyOperationSelected || SellOperationSelected) && ShareCode?.Length >= 4 && Quantity > 0 && OperationPrice > 0 && DateTime.TryParse(OperationDate, out DateTime a);
        }
    }
}
